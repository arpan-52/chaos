"""
Residual Computation.

Compute calibration residuals to assess solution quality.
"""

import numpy as np
from typing import Dict

from chaos.jones.operations import apply_jones


def compute_residuals(
    jones: np.ndarray,
    vis_obs: np.ndarray,
    vis_model: np.ndarray,
    antenna1: np.ndarray,
    antenna2: np.ndarray,
) -> Dict[str, float]:
    """
    Compute residual statistics after calibration.
    
    Residual = V_obs - J_i @ M @ J_j^H
    
    Parameters
    ----------
    jones : ndarray (n_ant, 2, 2)
        Jones matrices
    vis_obs : ndarray (n_bl, 2, 2)
        Observed visibilities
    vis_model : ndarray (n_bl, 2, 2)
        Model visibilities
    antenna1, antenna2 : ndarray (n_bl,)
        Antenna indices
    
    Returns
    -------
    stats : dict
        rms: overall RMS
        XX_rms, YY_rms, XY_rms, YX_rms: per-correlation RMS
        max_residual: maximum absolute residual
    """
    n_bl = len(antenna1)
    
    residuals = np.zeros((n_bl, 2, 2), dtype=np.complex128)
    
    for bl_idx in range(n_bl):
        a1, a2 = antenna1[bl_idx], antenna2[bl_idx]
        
        J_i = jones[a1]
        J_j = jones[a2]
        M = vis_model[bl_idx]
        V_obs = vis_obs[bl_idx]
        
        # V_pred = J_i @ M @ J_j^H
        V_pred = apply_jones(M, J_i, J_j)
        
        residuals[bl_idx] = V_obs - V_pred
    
    # Compute statistics
    res_abs = np.abs(residuals)
    
    stats = {
        "rms": np.sqrt(np.mean(res_abs**2)),
        "XX_rms": np.sqrt(np.mean(res_abs[:, 0, 0]**2)),
        "XY_rms": np.sqrt(np.mean(res_abs[:, 0, 1]**2)),
        "YX_rms": np.sqrt(np.mean(res_abs[:, 1, 0]**2)),
        "YY_rms": np.sqrt(np.mean(res_abs[:, 1, 1]**2)),
        "max_residual": res_abs.max(),
        "mean_residual": res_abs.mean(),
    }
    
    return stats


def compute_phase_residuals(
    jones: np.ndarray,
    vis_obs: np.ndarray,
    vis_model: np.ndarray,
    antenna1: np.ndarray,
    antenna2: np.ndarray,
) -> Dict[str, float]:
    """
    Compute phase residuals (in degrees) for diagonal terms.
    
    Parameters
    ----------
    jones : ndarray (n_ant, 2, 2)
    vis_obs : ndarray (n_bl, 2, 2)
    vis_model : ndarray (n_bl, 2, 2)
    antenna1, antenna2 : ndarray (n_bl,)
    
    Returns
    -------
    stats : dict
        XX_phase_rms, YY_phase_rms: phase RMS in degrees
    """
    n_bl = len(antenna1)
    
    phase_res_XX = []
    phase_res_YY = []
    
    for bl_idx in range(n_bl):
        a1, a2 = antenna1[bl_idx], antenna2[bl_idx]
        
        J_i = jones[a1]
        J_j = jones[a2]
        M = vis_model[bl_idx]
        V_obs = vis_obs[bl_idx]
        
        V_pred = apply_jones(M, J_i, J_j)
        
        # Phase residual for diagonal
        if np.abs(V_pred[0, 0]) > 1e-10:
            phase_diff_XX = np.angle(V_obs[0, 0]) - np.angle(V_pred[0, 0])
            phase_res_XX.append(np.degrees(phase_diff_XX))
        
        if np.abs(V_pred[1, 1]) > 1e-10:
            phase_diff_YY = np.angle(V_obs[1, 1]) - np.angle(V_pred[1, 1])
            phase_res_YY.append(np.degrees(phase_diff_YY))
    
    stats = {
        "XX_phase_rms": np.std(phase_res_XX) if phase_res_XX else 0.0,
        "YY_phase_rms": np.std(phase_res_YY) if phase_res_YY else 0.0,
    }
    
    return stats
